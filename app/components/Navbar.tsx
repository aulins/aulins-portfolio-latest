...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 w-full bg-slate-900/95 backdrop-blur-md border-b border-blue-500/30 shadow-lg shadow-blue-500/10 z-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div className="flex justify-between items-center">
                    ...
                    <div className="hidden md:flex space-x-8">
                        {navLinks.map((link) => (
                            <Link key={link.href} href={link.href} className={`transition-colors font-medium ${pathname === link.href ? "text-blue-400" : "text-slate-300 hover:text-blue-400"}`}>
                                {link.label}
                            </Link>
                        ))}
                    </div>
